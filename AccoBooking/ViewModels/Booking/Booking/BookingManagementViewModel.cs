...
namespace AccoBooking.ViewModels.Booking
{
  ...
  public class BookingManagementViewModel :
    BaseMasterViewModel<BookingSearchViewModel, BookingDetailViewModel, BookingListItem, DomainModel.Booking>
  {
    ...
    private Navigator _selectBookingNavigatorService;

    [ImportingConstructor]
    public BookingManagementViewModel(ExportFactory<BookingSearchViewModel> searchFactory,
      ExportFactory<SelectBookingViewModel> selectBookingFactory,
      ExportFactory<BookingDetailViewModel> detailFactory,
      ...
      : base(searchFactory, detailFactory, unitOfWorkManager, dialogManager, toolbar, bottomToolbar)
    {
      ...
      _selectBookingNavigatorService = new Navigator(this);
    }
    ...
    public async void StartDetail(int entityid)
    {
      ...
      await _navigator.NavigateToAsync(detail.GetType(),
        target =>
        {
          (target as BookingDetailViewModel).Start(entityid);
          (target as BookingDetailViewModel).Parent = this;
    ...
    public async void StartCreate(int entityid)
    {
      ...
      try
      {
        ...
        await _navigator.NavigateToAsync(detail.GetType(),
          target =>
          {
            (target as CreateBookingDetailViewModel).Start(true, _parentid);
            (target as CreateBookingDetailViewModel).Parent = this;
          }
          );
          ...
          else
          {
            using (ActiveDetail.Busy.GetTicket())
            {
              ...
              using (Busy.GetTicket())
              {
                ...
                PreviewMailViewModel.Acco =
                  await unitofWork.Accoes.WithIdFromDataSourceAsync(SessionManager.BookingAccoId);
                PreviewMailViewModel.Booking = await unitofWork.Bookings.WithIdFromDataSourceAsync(booking.BookingId);

                await
                  PreviewMailViewModel.SelectMailTemplateContent(MailTemplateType.Reservation, booking.BookerLanguageId);
    ...
    public async override Task Cancel()
    {
      if (Parent.GetType() == typeof (UpdateBookingViewModel) || Parent.GetType() == typeof (CreateBookingViewModel))
        if (ActiveEntity != null && ActiveUnitOfWork.HasChanges())
        {
          var dialogresult =
    ...
    public async override void Copy()
    {
      try
      {
        var selectBooking= _selectBookingFactory.CreateExport().Value;

        await _selectBookingNavigatorService.NavigateToAsync(
          selectBooking.GetType(),
          target =>
          {
            (target as SelectBookingViewModel).Parent = this;
            (target as SelectBookingViewModel).Start();
          });
      }
      catch (TaskCanceledException)
      {
