...
namespace Security
{
  ...
  public static class CryptoHelper
  {
    ...
    {
      DecryptFromStream(stream, key, cs =>
      {
        reader(cs);
        return true;
      });
    ...
    public static string Encrypt(this string dataToEncrypt)
    {
      ...
      string salt = PasswordSalt ?? DefaultPasswordSalt;
    ...
    public static string Decrypt(this string encryptedString)
    {
      ...
      using (MemoryStream decryptionStream = new MemoryStream())
      {
        // Create the crypto stream
        using (
          CryptoStream decrypt = new CryptoStream(decryptionStream, decryptor.CreateDecryptor(), CryptoStreamMode.Write)
          )
        {
          try
          {
